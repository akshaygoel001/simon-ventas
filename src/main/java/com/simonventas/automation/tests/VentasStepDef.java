...
public class VentasStepDef {
	...
	public VentasStepDef autosCrearCotizacion(int rowNum) {
		...
		a.enterTomador(rowNum, dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Num_Doc", rowNum), dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Placa", rowNum));
		a.datosVehiculo(dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Placa", rowNum),dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Marca_Placa", rowNum));
		a.datosDelConductorHabitual();
		a.datosDelBeneficiario();
		a.deducibles();
		a.crearCotizacion(rowNum,dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Cobertura", rowNum));
		return this;
	}
	
	@Step("Autos Emission Poliza {0}")
	public VentasStepDef autosEmissionPoliza(int rowNum) {
		AutosFlow a=new AutosFlow();
		a.doEmitir(rowNum, dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Color", rowNum), dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Emission_Placa", rowNum), dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Motor", rowNum), dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Chasis_Vin", rowNum), dataExcel.getCellData(PropertyManager.getConfigValueByKey("inputExcelSheetName"), "Tipo_Envio_Poliza", rowNum));
		return this;
	}
